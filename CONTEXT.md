...
*   **Fixed-Illumination Mobile Lattice**: A mobile-specific (< 1024px) background execution mode of the Isometric Cubes Wireframe Lattice that completely eliminates the `requestAnimationFrame` loop, pointer event listeners, and background pattern switcher. Instead of dynamic hover tracking, it renders a single-paint static spotlight with proximity-illuminated edges and vertex nodes in the upper viewport, updating statically on section scroll or theme change with zero runtime CPU/GPU overhead.
*   **Fixed Illumination Anchor**: A deterministic coordinate focal point (x: 65%, y: 18%) in mobile viewports that establishes a static proximity glow for nearby isometric cube edges and vertices without per-frame calculation or gesture interference.
